#ifndef _BOB3_UTILS_H_
#define _BOB3_UTILS_H_
...
inline static T1 constrain(T1 x, T2 a, T3 b) {
...
static unsigned int _do_count_pointers (void * p) {
  void ** pp = (void **)p;
  unsigned int r=0;
  while (*pp++) r++;
  return r;
...
template <typename T>
inline static unsigned int count_pointers(T ** p) {
  return _do_count_pointers(p);
}
