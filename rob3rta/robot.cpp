...
void Robot::setLed(uint8_t id, uint16_t color) {
    switch (id) {
        case 1:
        case 2:
...
uint16_t Robot::getLed(uint8_t id) {
    if (id<=2) {
...
uint16_t Robot::getIRSensor() {
    return analog_getValueExt(ANALOG_IR, 2);
}


uint16_t Robot::getIRLight() {
    return analog_getValueExt(ANALOG_IR, 0);
}


void Robot::enableIRSensor(bool enable) {
  
}


uint8_t Robot::getArm(uint8_t id) {
...
uint8_t Robot::getTouch(uint8_t id) {
    if ((id<=EAR_2) && (_arm_mode==ARMS_DETECTOR)) {
        return getArm(id);
    }
    switch(id) {
        case EAR_1:   return (analog_getValueExt(ANALOG_L0, 0) & 0x80)?1:0;
        case EAR_2:   return (analog_getValueExt(ANALOG_R0, 0) & 0x80)?1:0;
        case WHEEL_A: return (analog_getValueExt(ANALOG_L1, 0) & 0x80)?1:0;
        case WHEEL_B: return (analog_getValueExt(ANALOG_L3, 0) & 0x80)?1:0;
        case WHEEL_C: return (analog_getValueExt(ANALOG_R3, 0) & 0x80)?1:0;
        case WHEEL_D: return (analog_getValueExt(ANALOG_R1, 0) & 0x80)?1:0;
    }
    return 0;
}


void Robot::enableTouch(uint8_t enable) {
    _arm_mode = enable;
}


uint16_t Robot::getTemperature() {
    return analog_getValueExt(ANALOG_TEMP, 0)/4;
}


uint16_t Robot::getMillivolt() {
...
uint16_t colorcomp_hue(uint16_t color) {
    uint8_t r = colorcomp_red(color);
    uint8_t g = colorcomp_green(color);
    uint8_t b = colorcomp_blue(color);
    uint8_t mx = max(r,g,b);
    uint8_t mn = min(r,g,b);
    int16_t val;
    ...
    if (mx==mn) {
        ...
        // red
        val = ((int16_t)g-b)*60/(mx-mn) + 0;
    } else if (mx==g) {
        // green
        val = ((int16_t)b-r)*60/(mx-mn) + 120;
    } else {
        // blue
        val = ((int16_t)r-g)*60/(mx-mn) + 240;
    }
    if (val<0) val+=360;
    return val;
}

uint8_t colorcomp_sat(uint16_t color) {
    uint8_t r = colorcomp_red(color);
    uint8_t g = colorcomp_green(color);
    uint8_t b = colorcomp_blue(color);
    uint16_t mx = max(r,g,b);
    uint16_t mn = min(r,g,b);
    int16_t val = (mx - mn) * 100 / (mx + mn);
    return val;
}

uint8_t colorcomp_val(uint16_t color) {
    uint8_t val = 0;
    val += colorcomp_red(color);
    val += colorcomp_green(color);
    val += colorcomp_blue(color);
    val += (val+5)/9;
    return 2*val;
}
...
uint32_t random32() {
    ...
    ctx += analog_getRandomSeed(); // add some entropy...
    return random_r(&ctx);
}
